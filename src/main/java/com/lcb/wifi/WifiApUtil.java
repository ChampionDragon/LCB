...
public class WifiApUtil {
	...
	//便携式热点的状态为：10---正在关闭；11---已关闭；12---正在开启；13---已开启;14---热点创建失败
	...
	public boolean turnOnWifiAp(String str, String password,
		...
		if (Type == WifiSecurityType.WIFICIPHER_NOPASS) {
			...
			if (null != password && password.length() >= 8) {
				...
				ToastUtil.showLong("由于密码小于8位，我们自动设置成12345678，请及时修改");
	...
	public int getValidSecurity() {
		...
		if (DEBUG)
			Log.i(TAG, "getSecurity security="
		...
		} else if (configuration.allowedKeyManagement.get(4)) { // 4 means
																// WPA2_PSK
