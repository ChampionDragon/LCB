package com.lcb.view.GridPasswordView;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mTextColor == null)
            mTextColor = ColorStateList.valueOf(context.getResources().getColor(android.R.color.primary_text_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.gridPasswordView_gpvTextSize, -1);
        if (textSize != -1) {
            this.mTextSize = (int) DPUtil.px2sp(context, (float) textSize);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_gpvLineWidth, DPUtil.dip2px(getContext(), 1));
        ...
        //默认取0(numberPassword只能试用数字）如何想默认改成文本模式取1
        mPasswordType = ta.getInt(R.styleable.gridPasswordView_gpvPasswordType, 1);
