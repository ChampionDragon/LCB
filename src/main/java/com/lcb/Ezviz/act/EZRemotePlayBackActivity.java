...
import com.lcb.Ezviz.ActivityUtils;
import com.lcb.Ezviz.AudioPlayUtil;
import com.lcb.Ezviz.DataManager;
import com.lcb.Ezviz.EZUtils;
import com.lcb.Ezviz.RemoteFileTimeBar;
import com.lcb.Ezviz.RemoteListContant;
import com.lcb.Ezviz.RemoteListUtil;
import com.lcb.Ezviz.ScreenOrientationHelper;
import com.lcb.Ezviz.VerifyCodeInput;
import com.lcb.R;
...
public class EZRemotePlayBackActivity extends Activity implements View.OnClickListener, SurfaceHolder.Callback,
        Handler.Callback, TimeBarHorizontalScrollView.TimeScrollBarScrollListener, VerifyCodeInput.VerifyCodeInputListener {
...
//        mLandscapeTitleBar.setOnTouchListener(this);
        ...
        if (mAlarmStartTime == null) {
            ...
            mTitleBar.addTitleButton(R.drawable.remote_cal_selector, new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    showDatePicker();
                }
...
//            mRemotePlayBackEndTimeTv.setText(sdf.format(mAlarmStopTime.getTimeInMillis()));
            ...
            mRemotePlayBackSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                /**
                 * 拖动条停止拖动的时候调用
                 */
                @Override
...
//                handleSearchEZDeviceFileSuccess();
    ...
    private void handleSearchEZCloudFileSuccess(Calendar seletedTime) {
        if (mEZMediaPlayer == null) {
            mEZMediaPlayer = BaseApplication.getOpenSDK().createPlayer(mEZAlarmInfo.getDeviceSerial(), mEZAlarmInfo.getCameraNo());
            if (mEZMediaPlayer == null)
                return;
...
//        mRemotePlayBackCaptureRl.setVisibility(View.VISIBLE);
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    mCloudFileList = BaseApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(), mEZAlarmInfo.getCameraNo(), starttime, endtime);
    ...
    private void searchEZCloudFileList(final Calendar seletedTime) {
        new Thread() {
            ...
            public void run() {
                try {
                    mEZCloudFileList = BaseApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(), mEZAlarmInfo.getCameraNo(), mStartTime, mEndTime);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    mEZDeviceFileList = BaseApplication.getOpenSDK().searchRecordFileFromDevice(mEZAlarmInfo.getDeviceSerial(), mEZAlarmInfo.getCameraNo(), startTime, endTime);
                    if (mEZDeviceFileList != null && mEZDeviceFileList.size() > 0) {
                    ...
                    mEZCloudFileList = BaseApplication.getOpenSDK().searchRecordFileFromCloud(mEZAlarmInfo.getDeviceSerial(), mEZAlarmInfo.getCameraNo(), startTime, endTime);
